...
local function ConsiderQ()
	...
	then
		...
		then
			if mutils.CanKillTarget(target, nDamage, DAMAGE_TYPE_MAGICAL) == true or mutils.SafeIsChanneling(target) then
...
local function ConsiderW()
	...
	then
		...
		if mutils.IsValidTarget(target) 
			...
			and ( mutils.SafeGetAttackTarget(target) == bot or target:GetTarget() == bot ) 	
		then
