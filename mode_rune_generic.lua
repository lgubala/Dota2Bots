...
local runeStrategy = nil;
local strategyInitialized = false;

-- Add this function to generate random strategies
function InitializeRuneStrategy()
    if strategyInitialized then return end
    
    -- Generate a random strategy for this match
    local strategies = {
        "standard",      -- Each team goes to their safe runes (current behavior)
        "contest_power", -- Both teams contest power runes, split bounties
        "contest_bounty",-- Both teams contest one bounty, split others
        "all_mid",       -- Everyone goes to one power rune
        "cross_raid",    -- Teams send raiders to enemy bounties
        "mixed_chaos"    -- Random individual assignments
    };
    
    runeStrategy = strategies[RandomInt(1, #strategies)];
    strategyInitialized = true;
    
    -- Debug print (remove in production)
    print("[RUNE_STRATEGY] This match strategy: " .. runeStrategy);
end

function GetRuneAssignment()
    InitializeRuneStrategy();
    
    local assignment = {};
    
    if runeStrategy == "standard" then
        -- Original behavior
        if GetTeam() == TEAM_RADIANT then
            if bot:GetAssignedLane() == LANE_BOT then
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_2);
                assignment.contested = false;
            else
                assignment.location = GetRuneSpawnLocation(RUNE_POWERUP_1);
                assignment.contested = false;
            end
        else -- TEAM_DIRE
            if bot:GetAssignedLane() == LANE_TOP then
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_1);
                assignment.contested = false;
            else
                assignment.location = GetRuneSpawnLocation(RUNE_POWERUP_2);
                assignment.contested = false;
            end
        end
        
    elseif runeStrategy == "contest_power" then
        -- Both teams fight for power runes, split bounties safely
        if GetTeam() == TEAM_RADIANT then
            if bot:GetAssignedLane() == LANE_MID or bot:GetAssignedLane() == LANE_TOP then
                assignment.location = GetRuneSpawnLocation(RUNE_POWERUP_1); -- Top power rune
                assignment.contested = true;
            else
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_2); -- Safe bounty
                assignment.contested = false;
            end
        else -- TEAM_DIRE
            if bot:GetAssignedLane() == LANE_MID or bot:GetAssignedLane() == LANE_BOT then
                assignment.location = GetRuneSpawnLocation(RUNE_POWERUP_2); -- Bot power rune
                assignment.contested = true;
            else
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_1); -- Safe bounty
                assignment.contested = false;
            end
        end
        
    elseif runeStrategy == "contest_bounty" then
        -- Contest one bounty rune, others spread out
        local contestedBounty = (RandomInt(1,2) == 1) and RUNE_BOUNTY_1 or RUNE_BOUNTY_2;
        
        if contestedBounty == RUNE_BOUNTY_1 then
            -- Contest top bounty
            if bot:GetAssignedLane() == LANE_TOP then
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_1);
                assignment.contested = true;
    ...
    elseif runeStrategy == "all_mid" then
        -- Everyone goes to one power rune - pure chaos!
        local chosenRune = (RandomInt(1,2) == 1) and RUNE_POWERUP_1 or RUNE_POWERUP_2;
        assignment.location = GetRuneSpawnLocation(chosenRune);
        assignment.contested = true;
        
    elseif runeStrategy == "cross_raid" then
        -- Teams send some heroes to enemy territory
        if GetTeam() == TEAM_RADIANT then
            if bot:GetAssignedLane() == LANE_MID or (bot:GetAssignedLane() == LANE_TOP and RandomInt(1,2) == 1) then
                -- Raid enemy bounty
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_1);
                assignment.contested = true;
            elseif bot:GetAssignedLane() == LANE_BOT then
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_2); -- Defend home
                assignment.contested = false;
            else
                assignment.location = GetRuneSpawnLocation(RUNE_POWERUP_1); -- Safe power
                assignment.contested = false;
            end
        else -- TEAM_DIRE
            if bot:GetAssignedLane() == LANE_MID or (bot:GetAssignedLane() == LANE_BOT and RandomInt(1,2) == 1) then
                -- Raid enemy bounty
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_2);
                assignment.contested = true;
            elseif bot:GetAssignedLane() == LANE_TOP then
                assignment.location = GetRuneSpawnLocation(RUNE_BOUNTY_1); -- Defend home
                assignment.contested = false;
            else
                assignment.location = GetRuneSpawnLocation(RUNE_POWERUP_2); -- Safe power
                assignment.contested = false;
            end
        end
        
    else -- "mixed_chaos"
        -- Completely random assignments with some balance
        local allRunes = {RUNE_BOUNTY_1, RUNE_BOUNTY_2, RUNE_POWERUP_1, RUNE_POWERUP_2};
        local chosenRune = allRunes[RandomInt(1, #allRunes)];
        assignment.location = GetRuneSpawnLocation(chosenRune);
        assignment.contested = (RandomInt(1,3) <= 2); -- 66% chance of contest
    end
...
function GetDesire()
	...
	if not IsSuitableToPick() then		
		--bot:SetActiveMode(BOT_MODE_FARM);
		return BOT_MODE_DESIRE_NONE;
	end	
		...
		if closestRune == RUNE_BOUNTY_1 or closestRune == RUNE_BOUNTY_2 or closestRune == RUNE_BOUNTY_3 or closestRune == RUNE_BOUNTY_4 then
			...
			if DotaTime() > 3 * 60 + 50 then
				...
				elseif runeStatus == RUNE_STATUS_MISSING and DotaTime() > 60 and ( minute % 2 == 1 and sec > 52 ) and closestDist <= ProxDist then
					return CountDesire(BOT_MODE_DESIRE_MODERATE, closestDist, ProxDist);
				--elseif IsTeamMustSaveRune(closestRune) and runeStatus == RUNE_STATUS_UNKNOWN then
					--print("After 3 RUNE_STATUS_MISSING and save");
					--return CountDesire(BOT_MODE_DESIRE_MODERATE, closestDist, 5000);
				end
			end	
		end	
...
function Think()
	...
	if DotaTime() < 0 then 
        local runeAssignment = GetRuneAssignment();
        
        -- Optional: More aggressive movement if contested
        if runeAssignment.contested then
            -- Move more urgently to contested runes
            bot:Action_MoveToLocation(runeAssignment.location);
            -- Could add: bot:Action_UseAbility for movement abilities here
        else
            bot:Action_MoveToLocation(runeAssignment.location);
        end
        return
...
function CountDesire(base_desire, dist, maxDist)
	local desire = base_desire + RemapValClamped( dist, maxDist, 0, 0, 1-base_desire );
	return desire;
end	
...
function GetBotClosestRune()
	local cDist = 100000;	
	local cRune = -1;	
	for _,r in pairs(ListRuneLocations)
	do
...
function IsIBecameTheTarget(units)
	for _,u in pairs(units) do
		if mUtils.SafeGetAttackTarget(u) == bot then
			return true;
		end
	end
	return false;
