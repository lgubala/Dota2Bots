...
local function ConsiderQ()
	...
	if #enemies > 0 then
		for i=1, #enemies do
			if mutils.CanCastOnNonMagicImmune(enemies[i]) and mutils.SafeIsChanneling(enemies[i])
	...
	if mutils.IsRetreating(bot) then
		if #enemies > 0 then
			...
			local maxDmg = 0;	
			for i=1, #enemies do	
				local estDmg = mutils.SafeGetEstimatedDamageToTarget(enemies[i], true, bot, 2.0, DAMAGE_TYPE_ALL);
				if mutils.CanCastOnNonMagicImmune(enemies[i]) 
				   and estDmg >= maxDmg 
				   and mutils.SafeGetAttackTarget(enemies[i]) ~= nil
				then
					target = enemies[i];
					maxAD  = estDmg;
				end
	...
	then
		local target = mutils.SafeGetTarget(bot);
