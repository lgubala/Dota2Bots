...
function AbilityUsageThink()
	...
	if abilityHT == nil then abilityHT = npcBot:GetAbilityByName( "magnataur_horn_toss" ) end
	...
	if ( castHTDesire > 0 ) 
	then
		npcBot:Action_UseAbility( abilityHT );
		return;
	end
	if ( castDCDesire > 0 ) 
	then
...
function ConsiderHornToss()	
	...
	-- Make sure it's castable
	if ( not abilityHT:IsFullyCastable() ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end

	-- Get some of its values
	local nCastRange = abilityHT:GetCastRange();
	local nCastPoint = abilityHT:GetCastPoint( );
	local nManaCost  = abilityHT:GetManaCost( );
	local nDamage    = abilityHT:GetSpecialValueInt('damage');
