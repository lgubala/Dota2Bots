...
function ItemUsageModule.GetNonDisabledStrongestEnemy(bot, nCastRange)
	...
	for i=1, #units do
		...
		then
			local power = mutil.SafeGetEstimatedDamageToTarget(units[i], true, bot, 3.0, DAMAGE_TYPE_ALL );
...
function ItemUsageModule.ShouldTP(bot)
	...
	then
		if mutil.SafeGetHealth(bot) < 0.15*mutil.SafeGetMaxHealth(bot) and mutil.SafeWasRecentlyDamaged(bot, 2.0) and enemies == 0 then
			tpLoc = mutil.GetTeamFountain();
		elseif mutil.SafeGetHealth(bot) < 0.25*mutil.SafeGetMaxHealth(bot) and mutil.SafeWasRecentlyDamaged(bot, 3.0) and ItemUsageModule.CanJuke(bot) == true then
...
ItemUsageModule.Use['item_tango_single'] = function(item, bot, mode, extra_range)
	...
	if mutil.SafeGetHealth(bot) + total_heal <= mutil.SafeGetMaxHealth(bot) then
...
ItemUsageModule.Use['item_flask'] = function(item, bot, mode, extra_range)
	...
	if mutil.SafeGetHealth(bot) + total_heal <= mutil.SafeGetMaxHealth(bot)
	and mutil.SafeWasRecentlyDamaged(bot, 3.1) == false 
	then
...
ItemUsageModule.Use['item_faerie_fire'] = function(item, bot, mode, extra_range)
	...
	if ( mutil.IsRetreating(bot) and
		( mutil.SafeGetHealthPercent(bot)) < 0.15 ) 
		or DotaTime() > 10*60
	then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
	end
...
ItemUsageModule.Use['item_bottle'] = function(item, bot, mode, extra_range)
	...
	if bot:DistanceFromFountain() == 0 
		...
		and ( mutil.SafeGetHealth(bot) < mutil.SafeGetMaxHealth(bot) or bot:GetMana() < bot:GetMaxMana() ) 
	then
	...
	if charges > 0 then
		if mutil.SafeWasRecentlyDamaged(bot, 3.15) == false 
			and bot:HasModifier('modifier_bottle_regeneration') == false 
			and  ( mutil.SafeGetHealth(bot) + nHP*duration < 0.75*mutil.SafeGetMaxHealth(bot)
		...
		elseif bot.RuneType == RUNE_ILLUSION or bot.RuneType == RUNE_HASTE then
			...
			if mutil.IsRetreating(bot) 
				and mutil.SafeWasRecentlyDamaged(bot, 2.5) 
			then
				local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE)
				if #enemies > 0 then
					bot.RuneType = RUNE_INVALID;
		...
		elseif bot.RuneType == RUNE_REGENERATION then
			if mutil.IsRetreating(bot) 
				and mutil.SafeWasRecentlyDamaged(bot, 3.15) == false 
			then
				local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE)
				if #enemies == 0 then
					bot.RuneType = RUNE_INVALID;
...
ItemUsageModule.Use['item_magic_stick'] = function(item, bot, mode, extra_range)
	...
	local charges = item:GetCurrentCharges();
	local hp_ratio = mutil.SafeGetHealth(bot) / mutil.SafeGetMaxHealth(bot);
	...
	elseif charges > 1 
		...
		and ( mutil.SafeWasRecentlyDamaged(bot, 2.0) == true or bot:WasRecentlyDamagedByTower(3.0) == true )
	then
...
ItemUsageModule.Use['item_shadow_amulet'] = function(item, bot, mode, extra_range)
	...
	then	
		...
		if #towers == 0 then
			...
			for i=1, #enemies do
				if mutil.IsValidTarget(enemies[i])
					and ( mutil.SafeGetAttackTarget(enemies[i]) == bot
...
ItemUsageModule.Use['item_ghost'] = function(item, bot, mode, extra_range)
	if mutil.IsRetreating(bot) 
		and ( mutil.SafeWasRecentlyDamaged(bot, 2.5) == true or bot:WasRecentlyDamagedByTower(3.0) == true ) 
	then
		local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE)
		if #enemies > 0 then
			return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
...
ItemUsageModule.Use['item_unstable_wand'] = function(item, bot, mode, extra_range)
	if mutil.IsRetreating(bot)
		and ( mutil.SafeWasRecentlyDamaged(bot, 6.0) == true or bot:WasRecentlyDamagedByTower(6.0) == true )
	then	
		local enemies = bot:GetNearbyHeroes(1600, true, BOT_MODE_NONE)
		if #enemies == 0 then
			return BOT_ACTION_DESIRE_ABSOLUTE, nil,  'no_target';
...
ItemUsageModule.Use['item_pogo_stick'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot)
		and ( mutil.SafeWasRecentlyDamaged(bot, 5.0) == true or bot:WasRecentlyDamagedByTower(5.0) == true )
	then	
		local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE)
		if #enemies > 0 then
			return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
...
ItemUsageModule.Use['item_soul_ring'] = function(item, bot, mode, extra_range)
	if  mutil.IsGoingOnSomeone(bot) and mutil.SafeGetHealth(bot) - 150 > 0.25 * mutil.SafeGetMaxHealth(bot) and bot:GetMana() < 0.5 * bot:GetMaxMana() then
		local target = bot:GetTarget();
		if  mutil.IsValidTarget(target) 
...
ItemUsageModule.Use['item_power_treads'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot) == true and tread_stat ~= ATTRIBUTE_STRENGTH and mutil.SafeWasRecentlyDamaged(bot, 5.0) == true then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
	elseif mutil.IsRetreating(bot) == false and ItemUsageModule.IsHPHealing(bot) == true and mutil.SafeGetHealth(bot) < 0.95*mutil.SafeGetMaxHealth(bot) and tread_stat == ATTRIBUTE_STRENGTH then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';	
	elseif ItemUsageModule.IsHPHealing(bot) == false and mutil.SafeWasRecentlyDamaged(bot, 3.0) == false and mutil.IsRetreating(bot) == false then
		local enemies = bot:GetNearbyHeroes( 1200, true, BOT_MODE_NONE );
...
ItemUsageModule.Use['item_moon_shard'] = function(item, bot, mode, extra_range)
    -- Only consume if we DON'T already have the consumed buff
...
ItemUsageModule.Use['item_medallion_of_courage'] = function(item, bot, mode, extra_range)
	...
	for i=1, #allies do
		if allies[i]:GetUnitName() ~= bot:GetUnitName() 
			...
			and ( ( mutil.SafeGetHealth(allies[i])/mutil.SafeGetMaxHealth(allies[i]) < 0.35 and allies[i]:WasRecentlyDamagedByAnyHero(2.5) and mutil.CanCastOnNonMagicImmune(allies[i]) ) 
...
ItemUsageModule.Use['item_mekansm'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot) then
		local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE);
		if #enemies > 0 and mutil.SafeGetHealth(bot) + nHeal <= 0.5*mutil.SafeGetMaxHealth(bot) then
			return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
		end
	end
	...
	if mutil.IsInTeamFight(bot, 1300) then
		...
		local allies = bot:GetNearbyHeroes(nRadius, false, BOT_MODE_NONE);
		for i=1, #allies do
			if mutil.SafeGetHealth(allies[i])+ nHeal <= 0.5*mutil.SafeGetMaxHealth(allies[i]) then
				n_low_hp = n_low_hp + 1;
			end
		end
		if n_low_hp > 0 then
	...
	if charges >= 10 then
		...
		for i=1, #allies do
			...
			then
				...
				if  ( ( mode2 == BOT_MODE_RETREAT and mutil.SafeGetHealth(allies[i]) < 0.15 * mutil.SafeGetMaxHealth(allies[i]) ) 
						or ( mutil.SafeGetHealth(allies[i]) < 0.25 * mutil.SafeGetMaxHealth(allies[i]) 
							and ( (mutil.SafeGetAttackTarget( allies[i]) == nil ) or ( allies[i]:GetTarget() == nil ) ) ) )
				then	
					return BOT_ACTION_DESIRE_ABSOLUTE, allies[i], 'unit';
				end
			end
...
ItemUsageModule.Use['item_urn_of_shadows'] = function(item, bot, mode, extra_range)
	...
	then
		...
		if mutil.IsValidTarget(target) 
			...
			and (( item:GetName() == 'item_urn_of_shadows' and target:GetActualIncomingDamage( nDamage, DAMAGE_TYPE_MAGICAL  ) > mutil.SafeGetHealth(target) ) 
				or ( item:GetName() == 'item_spirit_vessel' and mutil.SafeGetHealth(target) < 0.75*mutil.SafeGetMaxHealth(bot) ) )
		then
			return BOT_ACTION_DESIRE_ABSOLUTE, target, 'unit';
		end
	end
	...
	if charges > 0 then
		...
		for i=1, #allies do
			if 	mutil.IsValidTarget(allies[i])
				...
				and mutil.CanCastOnNonMagicImmune(allies[i]) 
				and mutil.SafeGetHealth(allies[i]) + ( allies[i]:GetHealthRegen() + nRegen ) * nDuration < 0.65 * mutil.SafeGetMaxHealth(allies[i])   
			then
				return BOT_ACTION_DESIRE_ABSOLUTE, allies[i], 'unit';
			end
		end
...
ItemUsageModule.Use['item_veil_of_discord'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot) and mutil.SafeWasRecentlyDamaged(bot, 2.0)
	then
...
ItemUsageModule.Use['item_orchid'] = function(item, bot, mode, extra_range)
	...
	for i=1, #enemies do
		if mutil.IsValidTarget(enemies[i]) == true 
			...
			and mutil.SafeIsChanneling(enemies[i])
		then
...
ItemUsageModule.Use['item_force_staff'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot) == true
		and ( mutil.SafeWasRecentlyDamaged(bot, 3.0) == true or bot:WasRecentlyDamagedByTower(3.0) == true )
	then
		if item:GetName() == 'item_hurricane_pike' then
		...
		then
			if item:GetName() == 'item_hurricane_pike' then
				...
				if target_dist > bot_dist + 50 
					and ( target:IsFacingUnit(bot, 15) == true or mutil.SafeGetAttackTarget(target) == bot )  
...
ItemUsageModule.Use['item_cyclone'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot) == true
		and ( mutil.SafeWasRecentlyDamaged(bot, 2.0) == true
	...
	if item:GetName() == 'item_cyclone' then
		if ( bot:IsSilenced() == true or bot:IsRooted( ) == true )
			and mutil.SafeWasRecentlyDamaged(bot, 3.0) == true 
			and mutil.SafeGetHealth(bot) < 0.65*mutil.SafeGetMaxHealth(bot) 
		then
			return BOT_ACTION_DESIRE_ABSOLUTE, bot, 'unit';
		end
	end
...
ItemUsageModule.Use['item_harpoon'] = function(item, bot, mode, extra_range)
...
ItemUsageModule.Use['item_smoke_of_deceit'] = function(item, bot, mode, extra_range)
	
	-- Don't use smoke if already smoked
	if bot:HasModifier('modifier_smoke_of_deceit') then
		return BOT_ACTION_DESIRE_NONE;
	end
	
	-- Don't use smoke if enemies are nearby (smoke breaks)
	local enemies = bot:GetNearbyHeroes(1500, true, BOT_MODE_NONE);
	if #enemies > 0 then
		return BOT_ACTION_DESIRE_NONE;
	end
	
	-- ========== USE CASE 1: PRE-GAME BOUNTY RUNE STEALING ==========
	if DotaTime() < 0 and DotaTime() > -75 then
		-- Check if bot is assigned to steal bounty runes
		local shouldSteal = not role.CanBeSafeLaneCarry(bot:GetUnitName()) 
		...
		if shouldSteal then
			-- Count how many allies are stealing with us
			local stealCount = 0;
			for i, id in pairs(GetTeamPlayers(GetTeam())) do
				local unit = GetTeamMember(i);
				if IsPlayerBot(id) and unit ~= nil then
					local unitShouldSteal = not role.CanBeSafeLaneCarry(unit:GetUnitName()) 
						and unit:GetAssignedLane() ~= LANE_MID 
						and ( (GetTeam() == TEAM_RADIANT and unit:GetAssignedLane() == LANE_TOP) 
						   or (GetTeam() == TEAM_DIRE and unit:GetAssignedLane() == LANE_BOT) 
						   or role.IsSupport(unit:GetUnitName())
						);
					if unitShouldSteal then
						stealCount = stealCount + 1;
					end
				end
			end
			
			-- Use smoke if we have teammates to smoke with
			if stealCount >= 2 then
				-- Announce intention
				if bot.smokeBountyAnnounced == nil or bot.smokeBountyAnnounced == false then
					bot:ActionImmediate_Chat("Let's steal the bounty rune!", false);
					bot.smokeBountyAnnounced = true;
				end
				
				-- Wait a moment after announcing before using smoke
				if bot.smokeBountyAnnounced and DotaTime() > -70 then
					return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
				end
			end
		end
	...
	-- ========== USE CASE 2: GANKING / ROAMING ==========
	if (mode == BOT_MODE_ROAM or mode == BOT_MODE_GANK) and DotaTime() > 180 then
		-- Check if we have allies nearby for a smoke gank
		local allies = bot:GetNearbyHeroes(1200, false, BOT_MODE_NONE);
		local allyCount = 0;
		
		for _, ally in pairs(allies) do
			if not ally:IsIllusion() and ally:IsAlive() then
				allyCount = allyCount + 1;
			end
		end
		
		-- Use smoke if we have at least 2 allies (3+ heroes total)
		if allyCount >= 2 then
			-- Check that we're far from enemy vision
	...
	-- ========== USE CASE 3: TEAM SMOKE BEFORE OBJECTIVE ==========
	if DotaTime() > 600 then
		...
		-- Use smoke with 3+ allies when going for objectives
		if allyCount >= 3 then
			-- Check if we're near our fountain (safe place to smoke)
			if bot:DistanceFromFountain() < 2000 then
				-- Check if majority of team is ready
				local teamPlayers = GetTeamPlayers(GetTeam());
				local readyCount = 0;
				...
				for i, id in pairs(teamPlayers) do
					...
					if member ~= nil and not member:IsIllusion() and member:IsAlive() then
						if member:GetHealth() > member:GetMaxHealth() * 0.7 
							and member:GetMana() > member:GetMaxMana() * 0.5 
						then
				...
				-- Use smoke if at least 3 heroes are ready
	...
	-- ========== USE CASE 4: ESCAPE / JUKE (Emergency) ==========
	if mutil.IsRetreating(bot) 
		and mutil.SafeGetHealth(bot) < 0.3 * mutil.SafeGetMaxHealth(bot)
		and mutil.SafeWasRecentlyDamaged(bot, 10.0)
	then
		-- Only use as last resort if enemies lost vision
		...
		-- Use smoke to escape if we can juke
		if canJuke then
			local trees = bot:GetNearbyTrees(500);
			if #trees >= 3 then -- Near trees to hide
				return BOT_ACTION_DESIRE_HIGH, nil, 'no_target';
			end
		end
	end
	
	-- ========== USE CASE 5: ROSH ATTEMPT ==========
	if DotaTime() > 600 then
		local roshLoc = Vector(-2350, 1700, 0); -- Roshan pit location
		local distToRosh = GetUnitToLocationDistance(bot, roshLoc);
		
		if distToRosh < 3000 then
			...
			-- Use smoke with team before Rosh attempt
			if allyCount >= 3 and distToRosh > 1500 then
				local enemyTowers = bot:GetNearbyTowers(1800, true);
				if #enemyTowers == 0 then
					return BOT_ACTION_DESIRE_MODERATE, nil, 'no_target';
				end
...
ItemUsageModule.Use['item_ward_sentry'] = function(item, bot, mode, extra_range)
	local nCastRange = 500 + extra_range;
	local nTrueSightRange = 1050;
	local nSentryLifetime = 420; -- 7 minutes
	local myTeam = GetTeam();
	
	-- TEAM-SPECIFIC ward tracking
	...
	-- Helper: Distance between locations
	...
	-- Helper: Check if in combat with invisible enemies
	...
	-- Helper: Check if location has friendly tower vision (FIXED - only block friendly towers)
	local function IsInFriendlyTowerVision(location)
		local friendlyTowers = bot:GetNearbyTowers(2000, false);
		for _, tower in pairs(friendlyTowers) do
			if tower:IsAlive() and GetUnitToLocationDistance(tower, location) < 1400 then
				-- Exception: Allow if fighting invisible enemies nearby
	...
	-- Helper: Check if location is in enemy tower range (danger zone)
	local function IsInEnemyTowerRange(location)
		local enemyTowers = bot:GetNearbyTowers(2000, true);
		for _, tower in pairs(enemyTowers) do
			if tower:IsAlive() and GetUnitToLocationDistance(tower, location) < 900 then
				return true;
			end
		end
		return false;
	...
	local function HasSentryCoverage(location)
		...
		-- Check overlap with existing wards (reduced overlap threshold)
		for _, ward in pairs(teamWards) do
			local dist = GetDistanceBetweenLocations(location, ward.location);
			if dist < nTrueSightRange * 1.5 then -- Reduced from 2.0 to 1.5
				return true;
			end
		end
	...
	-- Helper: Check if location is in trees
	local function IsInTrees(location)
		local trees = bot:GetNearbyTrees(150);
		return #trees >= 3;
	end
	
	-- Helper: Find good placement location
	local function FindGoodPlacement(targetLoc, preferTrees, searchRadius)
		searchRadius = searchRadius or 300;
		
		-- Try tree locations first if requested
		if preferTrees then
			for i = 1, 24 do
				local angle = (i * 15) * (math.pi / 180);
				local testDist = searchRadius * 0.5 + (i % 8) * (searchRadius * 0.25);
				...
				if IsLocationPassable(testLoc) 
					and not IsInFriendlyTowerVision(testLoc)
					and not IsInEnemyTowerRange(testLoc)
					and not HasSentryCoverage(testLoc)
					and IsInTrees(testLoc) 
				then
					return testLoc;
				end
			end
	...
	-- Helper: Register ward placement
	local function RegisterWardPlacement(location)
		table.insert(_G.TeamSentryWards[myTeam], {location = location, time = DotaTime()});
	end
	
	-- Helper: Check if recent ward was placed by this bot
	local function WasRecentWardPlaced(timeLimit)
		if bot.lastSentryWardTime == nil then
			bot.lastSentryWardTime = -9999;
		end
		return (DotaTime() - bot.lastSentryWardTime) < timeLimit;
	end
	
	-- Helper: Update last ward time
	local function UpdateLastWardTime()
		bot.lastSentryWardTime = DotaTime();
	end
	
	-- ========== PRIORITY 1: IMMEDIATE INVISIBLE ENEMY (HIGHEST) ==========
	local enemyPids = GetTeamPlayers(GetOpposingTeam());
	for i = 1, #enemyPids do
		local info = GetHeroLastSeenInfo(enemyPids[i]);
		if IsHeroAlive(enemyPids[i]) and info ~= nil then
			local dInfo = info[1];
			if dInfo ~= nil 
				and dInfo.time_since_seen > 0.3
				and dInfo.time_since_seen < 2.0
				and GetUnitToLocationDistance(bot, dInfo.location) < 1200
			then
				local placeLoc = FindGoodPlacement(dInfo.location, false, 400);
	...
	-- ========== PRIORITY 2: TEAM FIGHT ==========
	if mutil.IsInTeamFight(bot, 1300) and not WasRecentWardPlaced(45) then
		local enemies = bot:GetNearbyHeroes(1200, true, BOT_MODE_NONE);
		if #enemies >= 2 then
			local centerLoc = bot:GetLocation();
			local placeLoc = FindGoodPlacement(centerLoc, false, 500);
			if placeLoc ~= nil then
				RegisterWardPlacement(placeLoc);
				UpdateLastWardTime();
				return BOT_ACTION_DESIRE_ABSOLUTE, placeLoc, 'point';
	...
	-- ========== PRIORITY 3: PUSHING (IMPROVED) ==========
	if mutil.IsPushing(bot) and not WasRecentWardPlaced(60) then
		local lane = bot:GetAssignedLane();
		local laneFrontLoc = GetLaneFrontLocation(myTeam, lane, 600); -- Further ahead
		
		-- Don't place if too far from action
		if GetUnitToLocationDistance(bot, laneFrontLoc) < 2000 then
			local placeLoc = FindGoodPlacement(laneFrontLoc, true, 500);
			if placeLoc ~= nil then
				RegisterWardPlacement(placeLoc);
				UpdateLastWardTime();
				return BOT_ACTION_DESIRE_ABSOLUTE, placeLoc, 'point';
	...
	-- ========== PRIORITY 4: DEFENDING (IMPROVED) ==========
	if mutil.IsDefending(bot) and not WasRecentWardPlaced(60) then
		local lane = bot:GetAssignedLane();
		local laneFrontLoc = GetLaneFrontLocation(myTeam, lane, -300); -- Defensive position
	...
	-- ========== PRIORITY 5: LANING PHASE ==========
	if mode == BOT_MODE_LANING and DotaTime() > 30 and DotaTime() < 600 and not WasRecentWardPlaced(90) then
		local lane = bot:GetAssignedLane();
		local laneLoc = GetLaneFrontLocation(myTeam, lane, -400);
		local placeLoc = FindGoodPlacement(laneLoc, true, 400);
		if placeLoc ~= nil then
			RegisterWardPlacement(placeLoc);
			UpdateLastWardTime();
			return BOT_ACTION_DESIRE_ABSOLUTE, placeLoc, 'point';
	...
	-- ========== PRIORITY 6: ROAMING/GANKING (MID-LATE GAME) ==========
	if (mode == BOT_MODE_ROAM or mode == BOT_MODE_GANK) and DotaTime() > 300 and not WasRecentWardPlaced(120) then
		local enemies = bot:GetNearbyHeroes(1800, true, BOT_MODE_NONE);
		if #enemies == 0 then
			local randomOffset = RandomVector(400);
			local placeLoc = FindGoodPlacement(bot:GetLocation() + randomOffset, true, 400);
			if placeLoc ~= nil then
				RegisterWardPlacement(placeLoc);
				UpdateLastWardTime();
				return BOT_ACTION_DESIRE_ABSOLUTE, placeLoc, 'point';
	...
	-- ========== PRIORITY 7: STRATEGIC WARD PLACEMENT (MID-LATE GAME) ==========
	if DotaTime() > 600 and not WasRecentWardPlaced(150) then
		-- Check if bot should place strategic wards
		local allies = bot:GetNearbyHeroes(1200, false, BOT_MODE_NONE);
		local enemies = bot:GetNearbyHeroes(1800, true, BOT_MODE_NONE);
		
		-- Place ward if safe and no immediate threats
		if #enemies == 0 and #allies <= 2 then
			local lane = bot:GetAssignedLane();
			local laneFrontLoc = GetLaneFrontLocation(myTeam, lane, 0);
			
			if GetUnitToLocationDistance(bot, laneFrontLoc) < 2500 then
				local placeLoc = FindGoodPlacement(laneFrontLoc, true, 600);
				if placeLoc ~= nil then
					RegisterWardPlacement(placeLoc);
					UpdateLastWardTime();
					return BOT_ACTION_DESIRE_ABSOLUTE, placeLoc, 'point';
...
ItemUsageModule.Use['item_disperser'] = function(item, bot, mode, extra_range)
	...
	-- Use on self when retreating to escape faster
	if mutil.IsRetreating(bot) == true
		and ( mutil.SafeWasRecentlyDamaged(bot, 3.0) == true or bot:WasRecentlyDamagedByTower(3.0) == true )
	then
		local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE);
	...
	-- Use on enemy when going on someone to slow them
	if mutil.IsGoingOnSomeone(bot)
	then
		local target = bot:GetTarget();
		if mutil.IsValidTarget(target) == true 
			and mutil.CanCastOnNonMagicImmune(target) == true
			and mutil.IsInRange(target, bot, nCastRange) == true
			and target:GetCurrentMovementSpeed() > 250
		then
			return BOT_ACTION_DESIRE_ABSOLUTE, target, 'unit';
		end
	end
	
	-- Use on ally to speed them up when they're retreating
	local allies = bot:GetNearbyHeroes(nCastRange, false, BOT_MODE_NONE);
	for i = 1, #allies do
		if allies[i]:GetUnitName() ~= bot:GetUnitName() 
			and mutil.CanCastOnNonMagicImmune(allies[i]) == true
		...
		then
			...
			if mode2 == BOT_MODE_RETREAT 
				and mutil.SafeGetHealth(allies[i]) < 0.5 * mutil.SafeGetMaxHealth(allies[i])
			then	
				return BOT_ACTION_DESIRE_ABSOLUTE, allies[i], 'unit';
			end
		end
	...
	-- Use on self when roaming/ganking for speed boost
	if mode == BOT_MODE_ROAM or mode == BOT_MODE_GANK 
	then
		local enemies = bot:GetNearbyHeroes(1500, true, BOT_MODE_NONE);
		if #enemies == 0 then
			return BOT_ACTION_DESIRE_ABSOLUTE, bot, 'unit';
...
ItemUsageModule.Use['item_gungir'] = function(item, bot, mode, extra_range)
	...
	then
		...
		-- Target specific enemy if no AoE opportunity
		local target = ItemUsageModule.GetNonDisabledStrongestEnemy(bot, nCastRange);
		if target ~= nil then
			return BOT_ACTION_DESIRE_ABSOLUTE, target:GetLocation(), 'point';
		end
	...
	-- Use in team fights for area control
	...
	-- Use when going on someone to root them
	if mutil.IsGoingOnSomeone(bot) then
		local target = bot:GetTarget();
		if mutil.IsValidTarget(target) 
			and mutil.CanCastOnNonMagicImmune(target) 
...
ItemUsageModule.Use['item_black_king_bar'] = function(item, bot, mode, extra_range)
	if mutil.IsRetreating(bot) == true
		and mutil.SafeWasRecentlyDamaged(bot, 2.0) == true
		and mutil.SafeGetHealth(bot) > 0.25 * mutil.SafeGetMaxHealth(bot)
		and mutil.SafeGetHealth(bot) < 0.65 * mutil.SafeGetMaxHealth(bot)
	then
		local enemies = bot:GetNearbyHeroes(1300, true, BOT_MODE_NONE);
		if #enemies > 0 then
			...
			if #enemies >= #allies then
				for i=1, #enemies do
					if mutil.IsValidTarget(enemies[i])
						and mutil.CanCastOnMagicImmune(enemies[i])
						and mutil.IsInRange(bot, enemies[i], enemies[i]:GetAttackRange()+150)
						and ( mutil.SafeGetAttackTarget(enemies[i]) == bot or enemies[i]:GetTarget() == bot
						or enemies[i]:IsFacingLocation(bot:GetLocation(), 15) )
					then
						return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
					end	
...
ItemUsageModule.Use['item_abyssal_blade'] = function(item, bot, mode, extra_range)
	...
	then
		...
		if tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 and mutil.SafeWasRecentlyDamaged(bot, 3.0) then
			local loc = mutil.GetEscapeLoc();
...
-- 	if mutil.IsRetreating(bot)
-- 		and mutil.SafeWasRecentlyDamaged(bot, 3.0) 
-- 	then
-- 		local enemies = bot:GetNearbyHeroes(nCastRange, true, BOT_MODE_NONE);
...
ItemUsageModule.Use['item_essence_ring'] = function(item, bot, mode, extra_range)
	local hpRes = 425;
	if mutil.IsRetreating(bot)
		and mutil.SafeGetHealth(bot) < 0.35 * mutil.SafeGetMaxHealth(bot)
		and ( mutil.SafeWasRecentlyDamaged(bot, 3.0) or bot:WasRecentlyDamagedByTower(3.0) )
	then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
	end
	return BOT_ACTION_DESIRE_NONE;
...
-- 	if mutil.IsRetreating(bot) == true
-- 		and mutil.SafeWasRecentlyDamaged(bot, 0.5) == true
-- 	then
-- 		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
-- 	end
-- 	return BOT_ACTION_DESIRE_NONE;
...
ItemUsageModule.Use['item_ex_machina'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot) and mutil.SafeWasRecentlyDamaged(bot, 1.0) 
	then
		local nCdItem = 0;
		for i=0, 5 do
			local cdIt = bot:GetItemInSlot(i);
...
ItemUsageModule.Use['item_cheese'] = function(item, bot, mode, extra_range)
	...
	if mutil.SafeGetHealth(bot) +0.5*maxHP < mutil.SafeGetMaxHealth(bot)
