...
function AbilityUsageThink()
	...
	if abilityD == nil then abilityD = bot:GetAbilityByName( "jakiro_liquid_ice" ) end
	...
	if castDDesire > 0 then
		bot:Action_UseAbilityOnEntity(abilityD, DTarget);		
		return
	end
...
function ConsiderE()
	...
	local target  = bot:GetTarget(); 
	local aTarget = mutil.SafeGetAttackTarget(bot); 
	local enemies = bot:GetNearbyHeroes(castRange, true, BOT_MODE_NONE);
...
function ConsiderD()
	...
	if not abilityD:IsFullyCastable() or abilityD:IsHidden() then
		return BOT_ACTION_DESIRE_NONE;
	end
