...
local castArmDesire = 0;
local castFlameDesire = 0;
...
local abilityArm = nil;
local abilityFlame = nil;
...
function AbilityUsageThink()
	...
	if(castFlameDesire > 0)
	then
		npcBot:Action_UseAbility(abilityFlame);
		return;
	end	

	if(castArmDesire > 0)
	then
		npcBot:Action_UseAbility(abilityArm);
		return;
	end	
...
function ConsiderFlameThrow()	
	
	if not abilityFlame:IsFullyCastable() or abilityFlame:IsHidden() then
		return BOT_ACTION_DESIRE_NONE;
	end

	--print("#######CONSIDA");
	local nRadius = abilityArm:GetSpecialValueInt("radius");

	if mutil.IsGoingOnSomeone(npcBot) then
		local npcTarget = npcBot:GetTarget();
		if mutil.IsValidTarget(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nRadius) then
