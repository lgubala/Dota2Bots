...
function ConsiderLivingArmor()
	...
	then
		...
		do
			if mutil.CanCastOnNonMagicImmune(npcAlly) and mutil.SafeGetHealthPercent(npcAlly) < 0.25 
			then
...
function ConsiderFireblast()
	...
	then
		...
		if ( #tableNearbyEnemyHeroes >= 3 ) then
			...
			do
				...
				then
					local nHealth = mutil.SafeGetHealth(npcEnemy)
	...
	for _,npcEnemy in pairs( tableNearbyEnemyHeroes )
	do
		if ( mutil.SafeIsChanneling(npcEnemy) and  mutil.CanCastOnMagicImmune( npcEnemy ) and not mutil.IsDisabled(true, npcEnemy) ) 
		then
