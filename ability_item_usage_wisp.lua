...
function ConsiderDeathCoil()
	...
	then
		...
		do
			...
			then
				local nAllyHP = mutil.SafeGetHealth(npcAlly);
				if ( ( nAllyHP < nLowestHealth and mutil.SafeGetHealthPercent(npcAlly) < 0.5 ) or isDisabled(npcAlly) )
				then
...
function ConsiderRelocate()
	...
	then
		...
		do
			...
			then
				local nAllyHP = mutil.SafeGetHealth(npcAlly);
				if npcAlly:HasModifier('modifier_wisp_tether') and ( ( nAllyHP < nLowestHealth and mutil.SafeGetHealthPercent(npcAlly) < 0.35 ) or isDisabled(npcAlly) )
				then
					nLowestHealth = nAllyHP;
					lowHpAlly = npcAlly;
				end
