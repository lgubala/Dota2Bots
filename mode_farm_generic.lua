	...
	if farmLane then
		...
		if target ~= nil then
			...
			if bot:WasRecentlyDamagedByTower(1.0) or bot:WasRecentlyDamagedByCreep(1.0) then
				...
				return
			elseif  mutil.SafeGetEstimatedDamageToTarget(bot, false, target, t, DAMAGE_TYPE_PHYSICAL) >= target:GetHealth() then
