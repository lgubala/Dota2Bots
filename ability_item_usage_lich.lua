		...
		if #allies > 0 and #enemies >= 1 then
			for i=1,#allies do
				if mutils.CanCastOnNonMagicImmune(allies[i])
				   ...
				   and ( mutils.SafeGetAttackTarget(allies[i]) == nil or mutils.IsRetreating(allies[i]) )
				   and mutils.SafeGetHealth(allies[i]) <= minHP
     			   and mutils.SafeGetHealth(allies[i]) <= 0.5*mutils.SafeGetMaxHealth(allies[i]) 
				then
					weakest = allies[i];
					minHP = mutils.SafeGetHealth(allies[i]);
				end
			end
		end
		if weakest ~= nil then
