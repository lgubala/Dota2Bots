...
local shardIndex = nil;
local shardRemoved = false;
--Check if item list contains aghanims shard a remove it, so we put it back after 15 minutes (10 in turbo)
--local shardIndex = nil
--local shardIsBackOnBuyList = false;
--local shardTime = 0;
--
...
--end	
--
--for i, item in ipairs(bot.itemToBuy) do
    --if item == "item_aghanims_shard" then
      --shardIndex = i;
      --break;
    --end
--end
...
--if shardIndex ~= nil then
  --table.remove(bot.itemToBuy, shardIndex)
--end
...
if DotaTime() < 0 then
	...
	if role.IsSupport(unitName) then
	...
	if ( bot:GetGold() >= cost ) then
		...
		if bot.SecretShop and courier ~= nil and GetCourierState(courier) == COURIER_STATE_IDLE and courier:DistanceFromSecretShop() == 0 then
			if courier:ActionImmediate_PurchaseItem( bot.currentComponentToBuy ) == PURCHASE_ITEM_SUCCESS then
				if bot.currentComponentToBuy == "item_aghanims_shard" then
					hasShard = true;
				end		
				bot.currentComponentToBuy = nil;
...
function ItemPurchaseThink()  
	...
	--Add aghanim shard back if we had one in list
	--if shardIndex ~= nil and DotaTime() > shardTime * 60  and shardIsBackOnBuyList == false then
		--bot.currListItemToBuy[#bot.currListItemToBuy+1]  =  "item_aghanims_shard";
		--bot.currentComponentToBuy = nil;
		--shardIsBackOnBuyList = true;
		--return
	--end
