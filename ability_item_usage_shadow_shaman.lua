...
function ConsiderShackles()
	...
	for _,npcEnemy in pairs( tableNearbyEnemyHeroes )
	do
		if ( mutil.SafeIsChanneling(npcEnemy) and mutil.CanCastOnNonMagicImmune(npcEnemy) and not mutil.IsDisabled(true, npcEnemy) ) 
		then
			return BOT_ACTION_DESIRE_MODERATE, npcEnemy;
		end
	end
...
function ConsiderCancelShackle()
	...
	if not mutil.SafeIsChanneling(npcBot) then return BOT_MODE_NONE; end
