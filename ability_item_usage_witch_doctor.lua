	...
	if bot:GetMana() > GetReservedMana() then
		...
		for i=1, #allies do
			if mutils.IsValidTarget(allies[i])
				...
				and mutils.SafeGetHealth(allies[i]) < 0.55*allies[i]:GetMaxHealth()
			then
...
local function ConsiderR()
	...
	then
		...
		if mutils.IsValidTarget(target) 
			...
			and mutils.SafeGetHealth(target) > 0.5*nDamage
		then
...
function AbilityUsageThink()
	...
	if mutils.SafeIsChanneling(bot) 
		and mutils.CanBeCast(abilities[2]) == true 
		and mutils.SafeGetHealth(bot) < 0.55*bot:GetMaxHealth()
