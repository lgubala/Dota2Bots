...
function GetTargetBuildingToHeal(total_heal)
	...
	for i=1, #mutil.towers do
		...
		if tower ~= nil and tower:HasModifier('modifier_treant_living_armor') == false then
			local hp = mutil.SafeGetHealth(tower);
	...
	for i=1, #mutil.barracks do
		...
		if barrack ~= nil and barrack:HasModifier('modifier_treant_living_armor') == false then
			local hp = mutil.SafeGetHealth(barrack);
...
function ConsiderLivingArmor()
	...
	then
		...
		do
			if (  mutil.CanCastOnNonMagicImmune(npcAlly) and( mutil.SafeGetHealthPercent(npcAlly) ) < 0.5 and npcAlly:HasModifier('modifier_treant_living_armor') == false ) 
			then
	...
	then
		local npcTarget = npcBot:GetTarget();
		if mutil.IsValidTarget(npcTarget) and ( mutil.SafeGetHealthPercent(npcBot) ) < 0.5  and mutil.IsInRange(npcTarget, npcBot, 600) 
		then
			return BOT_ACTION_DESIRE_MODERATE, npcBot;
		end
	end
	...
	do
		...
		if Player:IsAlive() and Player:HasModifier('modifier_treant_living_armor') == false and mutil.SafeGetHealthPercent(Player) < 0.65 and 
