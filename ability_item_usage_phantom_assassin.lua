...
function AbilityUsageThink()
	...
	castRDesire = ConsiderFanOfKnives(); -- Add Fan of Knives
	
	local desires = "Desires - Q: " .. castQDesire .. " W: " .. castWDesire .. " E: " .. castEDesire .. " FoK: " .. castRDesire;
	--print("[PA] " .. desires);
	
	if castRDesire > 0 then
		--print("[PA] Using Fan of Knives!");
		bot:Action_UseAbility(abilities[4]);		
		return
	end
	
	if castQDesire > 0 then
		--print("[PA] Using Stifling Dagger on " .. targetQ:GetUnitName());
		bot:Action_UseAbilityOnEntity(abilities[1], targetQ);		
		return
	end
	
	if castWDesire > 0 then
		--print("[PA] Using Phantom Strike on " .. targetW:GetUnitName());
		bot:Action_UseAbilityOnEntity(abilities[2], targetW);		
		return
	end
	
	if castEDesire > 0 then
		--print("[PA] Using Blur for evasion!");
...
function ConsiderE()
	...
	-- Use when retreating and taking damage
	if mutils.IsRetreating(bot) and bot:WasRecentlyDamagedByAnyHero(3.0) then
		--print("[PA] BLUR - Retreating and taking damage");
		return BOT_ACTION_DESIRE_HIGH;
	end
	
	-- Use when low HP and enemies nearby
	if bot:GetHealth() < 0.5 * bot:GetMaxHealth() then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(800, true, BOT_MODE_NONE);
		if #tableNearbyEnemyHeroes >= 1 then
			--print("[PA] BLUR - Low HP with " .. #tableNearbyEnemyHeroes .. " enemies nearby");
			return BOT_ACTION_DESIRE_HIGH;
		end
	end
	
	-- Use when going on someone and taking damage from multiple enemies
	if mutils.IsGoingOnSomeone(bot) and bot:WasRecentlyDamagedByAnyHero(2.0) then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(600, true, BOT_MODE_NONE);
		if #tableNearbyEnemyHeroes >= 2 then
			--print("[PA] BLUR - Fighting multiple enemies (" .. #tableNearbyEnemyHeroes .. ")");
...
function ConsiderFanOfKnives()
	...
	local nRadius = 550; -- From ability data
	local manaCost = abilities[4]:GetManaCost();
	
	--print("[PA] Fan of Knives check - Radius: " .. nRadius .. " ManaCost: " .. manaCost);
	
	-- TEAMFIGHT: Use when multiple enemies nearby
	if mutils.IsInTeamFight(bot, 1200) then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(nRadius, true, BOT_MODE_NONE);
		if #tableNearbyEnemyHeroes >= 2 then
			--print("[PA] TEAMFIGHT Fan of Knives - " .. #tableNearbyEnemyHeroes .. " enemies");
			return BOT_ACTION_DESIRE_HIGH;
		elseif #tableNearbyEnemyHeroes >= 1 then
			--print("[PA] TEAMFIGHT Fan of Knives - Single target");
			return BOT_ACTION_DESIRE_MODERATE;
		end
	end
	
	-- GOING ON SOMEONE: Use when chasing target
	if mutils.IsGoingOnSomeone(bot) then
		local target = bot:GetTarget();
		if mutils.IsValidTarget(target) and mutils.IsInRange(target, bot, nRadius) then
			local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(nRadius, true, BOT_MODE_NONE);
			if #tableNearbyEnemyHeroes >= 1 then
				--print("[PA] CHASE Fan of Knives on " .. target:GetUnitName() .. " + " .. (#tableNearbyEnemyHeroes-1) .. " others");
				return BOT_ACTION_DESIRE_HIGH;
			end
		end
	end
	
	-- FARMING: Use on creep waves when safe
	if (bot:GetActiveMode() == BOT_MODE_LANING or mutils.IsPushing(bot)) and mutils.CanSpamSpell(bot, manaCost) then
		local tableNearbyCreeps = bot:GetNearbyLaneCreeps(nRadius, true);
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(1200, true, BOT_MODE_NONE);
		if #tableNearbyCreeps >= 3 and #tableNearbyEnemyHeroes == 0 then
			--print("[PA] FARMING Fan of Knives - " .. #tableNearbyCreeps .. " creeps");
			return BOT_ACTION_DESIRE_LOW;
		end
	end
	
	-- RETREAT: Use when retreating and enemies are close (slow them down)
	if mutils.IsRetreating(bot) and bot:WasRecentlyDamagedByAnyHero(2.0) then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(nRadius, true, BOT_MODE_NONE);
		if #tableNearbyEnemyHeroes >= 1 then
			--print("[PA] RETREAT Fan of Knives - Slowing " .. #tableNearbyEnemyHeroes .. " pursuers");
			return BOT_ACTION_DESIRE_MODERATE;
		end
	end
