...
function ConsiderW()
	...
	then
		if abilities[1]:IsFullyCastable() then 
			...
			for i=1,#enemies do
				if mutils.IsValidTarget(enemies[i]) and mutils.CanCastOnNonMagicImmune(enemies[i]) and mutils.SafeGetHealth(enemies[i]) < nDamage + nDamage2 then
...
function ConsiderE()
	...
	if mutils.IsRetreating(bot) == false and mutils.SafeGetHealth(bot) > 0.15*bot:GetMaxHealth() and bot:DistanceFromFountain() > 1000 then
...
function ConsiderE2()
	...
	then
		...
		if mutils.IsValidTarget(target) and mutils.CanCastOnNonMagicImmune(target) 
			...
			and bot:GetAttackDamage() >= mutils.SafeGetHealth(target)
		then
			return BOT_ACTION_DESIRE_HIGH, target;
		end
	end
