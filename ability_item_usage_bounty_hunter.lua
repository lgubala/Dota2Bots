...
function AbilityUsageThink()
	...
	castEDesire,  castETarget, hasShard = ConsiderE();
...
function ConsiderQ()
	...
	do
		if mutil.SafeIsChanneling(npcEnemy) then
...
function ConsiderE()
	...
	local baseCooldown = abilityE:GetSpecialValueInt("AbilityCooldown");
	if baseCooldown == 13 or baseCooldown == 12 or baseCooldown == 11 or baseCooldown == 10 then
		gotShard = true;
	else
		gotShard = false;
	end
	...
	if gotShard then
		...
		then
			...
			do
				...
				then
					return BOT_ACTION_DESIRE_HIGH , npcBot, "HASshard";
				end
			end
		end
		...
		for _,myFriend in pairs(tableNearbyFriendlyHeroes) do
			...
			then
				return BOT_ACTION_DESIRE_HIGH , myFriend,"HASshard";
			end
		end	
		...
		then
			...
			then
				...
				return BOT_ACTION_DESIRE_HIGH , ClosestBot,"HASshard";
			end	
		end
	else

		if npcBot:HasModifier('modifier_bounty_hunter_wind_walk') then
			return BOT_ACTION_DESIRE_NONE;
		end
		    -- Get some of its values
		local nCastPoint = abilityQ:GetCastPoint( );
		...
		then
			...
			then
				return BOT_ACTION_DESIRE_MODERATE ,0, "NOshard";
			end
		end
