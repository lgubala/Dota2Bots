...
function ConsiderEnrage()
	...
	if  npcBot:GetActiveMode() == BOT_MODE_FARM and mutil.SafeGetHealthPercent(npcBot) < 0.20
	then
		local npcTarget = mutil.SafeGetAttackTarget(npcBot);
		if npcTarget ~= nil then
			return BOT_ACTION_DESIRE_LOW;
	...
	if ( npcBot:GetActiveMode() == BOT_MODE_ROSHAN  ) and mutil.SafeGetHealthPercent(npcBot) < 0.65
	then
		local npcTarget = mutil.SafeGetAttackTarget(npcBot);
		if ( mutil.IsRoshan(npcTarget) and mutil.CanCastOnMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, 300)  )
		then
	...
	-- If we're going after someone
	if mutil.IsGoingOnSomeone(npcBot) and mutil.SafeGetHealthPercent(npcBot) < 0.65
	then
		local npcTarget = npcBot:GetTarget();
