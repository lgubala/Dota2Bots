...
function X.UpdateTeamStatus()
	...
	local pow = mutil.SafeGetEstimatedDamageToTarget(npcBot, true, GetTeamMember( 1 ), 10.0, DAMAGE_TYPE_PHYSICAL ) / dummyPhysResist
	print("pow test "..pow)
	if npcBot.attackPower == nil or pow > npcBot.attackPower then
		npcBot.attackPower = pow
	end
	pow = mutil.SafeGetEstimatedDamageToTarget(npcBot, true, GetTeamMember( 1 ), 10.0, DAMAGE_TYPE_MAGICAL ) / dummyMagResist
