...
local function TestFacetFunctions()
    local potential_functions = {
        "SelectFacet",
        "SelectHeroFacet", 
        "PickFacet",
        "SetFacet",
    ...
    print("[FACET DISCOVERY] Starting function probe...")
    
    for _, func_name in pairs(potential_functions) do
        if _G[func_name] ~= nil then
            print("[FACET FOUND] Function exists: " .. func_name .. " Type: " .. type(_G[func_name]))
        end
    end
    
    -- Test if SelectHero accepts a third parameter for facets
    print("[FACET TEST] Testing SelectHero with extra parameters...")
    local success, err = pcall(function()
        -- Don't actually select, just test the function signature
        return SelectHero(99, "npc_dota_hero_axe", 0)
    end)
    if success then
        print("[FACET TEST] SelectHero accepts 3 parameters! Facets might work!")
    else
        print("[FACET TEST] SelectHero with 3 params failed: " .. tostring(err))
    end
    
    -- Test GetHeroFacets if it exists
    if GetHeroFacets then
        local facets = GetHeroFacets("npc_dota_hero_axe")
        if facets then
            print("[FACET DATA] Axe facets: " .. tostring(facets))
        end
    end
end
	...
	if GetHeroPickState() == 60 and GameTime() >= 15 and GameTime() >= lastpick + 2 then
		for i,id in pairs(GetTeamPlayers(GetTeam())) do
			if IsPlayerBot(id) and IsPlayerInHeroSelectionControl(id) and GetSelectedHeroName(id) == "" then
                ...
                if special_mode then
                    -- Check which team the bot belongs to
                    local team = GetTeamForPlayer(id)
                    if team == TEAM_RADIANT and #radiantHeroList > 0 then
                        -- Pick a random hero from radiant list and remove it
                        local randomIndex = RandomInt(1, #radiantHeroList)
                        hero = radiantHeroList[randomIndex]
                        table.remove(radiantHeroList, randomIndex)
                        print("Radiant bot picking: " .. hero)
                    elseif team == TEAM_DIRE and #direHeroList > 0 then
                        -- Pick a random hero from dire list and remove it
                        local randomIndex = RandomInt(1, #direHeroList)
                        hero = direHeroList[randomIndex]
                        table.remove(direHeroList, randomIndex)
                        print("Dire bot picking: " .. hero)
                    else
                        -- If the hero lists are empty, fall back to normal picking
                        if testMode then
                            hero = GetRandomHero()
                        else
                            hero = PickRightHero(i - 1)
                        end
                        print("Fallback picking: " .. hero)
                    end
                else
                    -- Normal hero picking logic when special_mode is false
                    if testMode then
                        hero = GetRandomHero()
                    else
                        hero = PickRightHero(i - 1)
...
function PicksHero()
	...
	elseif	PickCycle == 5 then
		...
		print("PICKED " ..PickedHero.."for role carry" );
	end
...
function PickRightHero(slot)
	...
	if slot == 0 then
		...
		print("PICKED " ..initHero.."for role Mid" );
	elseif slot == 1 then
		...
		print("PICKED " ..initHero.."for role OFF or carry" );
	elseif slot == 2 then
		while not role.CanBeSupport(initHero) do
			initHero = GetRandomHero();
		end
		print("PICKED " ..initHero.."for role CARRY or carry" );
	elseif slot == 3 then
		while not role.CanBeSupport(initHero) do
			initHero = GetRandomHero();
		end
