...
function ConsiderW()
	...
	do
		if mutils.SafeIsChanneling(enemy) and mutils.CanCastOnNonMagicImmune(enemy) and bot:IsFacingUnit(enemy, 10) then
	...
	do
		...
		then
			...
			if  ( mode2 == BOT_MODE_RETREAT  
				or mutils.SafeGetAttackTarget(allies[i]) == nil 
...
function ConsiderE()
	...
	local target  = bot:GetTarget(); 
	local aTarget = mutils.SafeGetAttackTarget(bot); 
	local enemies = bot:GetNearbyHeroes(castRange, true, BOT_MODE_NONE);
		...
		then
			...
			if  mode2 == BOT_MODE_RETREAT 
					or ( allies[i]:GetHealth() < 0.15 * allies[i]:GetMaxHealth() 
						and ( (mutils.SafeGetAttackTarget( allies[i]) == nil ) or ( allies[i]:GetTarget() == nil ) ) ) 
			then	
				gobled_unit = 'hero'
	...
	then
		...
		then
			...
			if #targetAllies >= 1 then
				print("### SNAPING " .. tostring(target:GetLocation()));
				return BOT_ACTION_DESIRE_HIGH, target:GetLocation();
			end
		end
	end
