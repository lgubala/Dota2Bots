...
function AbilityUsageThink()
	...
	if abilityFire == nil then abilityFire = npcBot:GetAbilityByName( "dragon_knight_fireball" ) end
	
	castQDesire, castQLoc    = ConsiderQ();
	castWDesire, castWTarget = ConsiderW();
	castRDesire              = ConsiderR();
	castFireDesire, castFireLoc = ConsiderFire();

	if ( castFireDesire > 0 ) 
	then
		
		npcBot:Action_UseAbilityOnLocation( abilityFire, castFireLoc );
		return;
	end
...
function ConsiderW()
	...
	for _,npcEnemy in pairs(tableNearbyEnemyHeroes)
	do
		if mutil.CanCastOnNonMagicImmune(npcEnemy) and ( mutil.CanKillTarget(npcEnemy, nDamage, DAMAGE_TYPE_MAGICAL) or mutil.SafeIsChanneling(npcEnemy) ) then
...
function ConsiderFire()
	-- Make sure it's castable

	if not abilityFire:IsFullyCastable() or abilityFire:IsHidden() then
		return BOT_ACTION_DESIRE_NONE , 0;
	end
	...
	if npcBot:HasModifier( 'modifier_dragon_knight_dragon_form' ) then
		nCastRange = abilityFire:GetSpecialValueInt( 'dragon_cast_range' );
	else
	...
	local nCastPoint = abilityFire:GetCastPoint( );
	local nManaCost  = abilityFire:GetManaCost( );
	local nDamage    = abilityFire:GetAbilityDamage( );
	...
	then
		local npcTarget = mutil.SafeGetAttackTarget(npcBot);
		if npcTarget ~= nil and not npcTarget:IsBuilding() then
			return BOT_ACTION_DESIRE_VERYHIGH, npcTarget:GetLocation();
		end
	end	
