...
local tTalks = require(GetScriptDirectory() ..  "/TrashTalk");
local pingUtil = require(GetScriptDirectory() .. "/PingUtility");
local killTracker = require(GetScriptDirectory() .. "/KillTracker");
local courierUtil = require(GetScriptDirectory() .. "/CourierUtility");
local mutil = require(GetScriptDirectory() ..  "/MyUtility");
local bot = GetBot();


local dota= false;
local lastAttackCheck = -90;
local lastEnemyCheck = -90;

--trashtalk
local trashTalkedOnStart = false;
local trashTalkedOnBegining = false;
local trashTalkedInMid = false;
local trashTalkedOnEnd = false;

local getOffensiveMessage = tTalks.getTrashtalk("Offensive");
local getDefensiveMessage = tTalks.getTrashtalk("Defensive");
local getNeutralMessage = tTalks.getTrashtalk("Neutral");
...
if bot:IsHero() then
	randMinA = RandomInt(1,5);
  	randMinB = RandomInt(10,15);
  	randMinC = RandomInt(20,25);
  	randMinD = RandomInt(30,35);  
	
  	randSecA = RandomInt(0,60);
  	randSecB = RandomInt(0,60);
  	randSecC = RandomInt(0,60);
  	randSecD = RandomInt(0,60);
  	--local name = GetHumanPlayerName();
  	--print("human is: "..tostring(name))
  	local players = GetTeamPlayers(TEAM_RADIANT)
	for _, playerID in pairs(players) do
  	...
  	--print("RAnd M " ..randMinA .. "rand S ".. randSecA .. "hero" .. tostring(bot:GetUnitName()));
...
function SafeGetAbilityByName(bot, abilityName)
    ...
    local success, ability = pcall(function() return bot:GetAbilityByName(abilityName) end)
    if success then
        return ability
    else
...
function OnKillHero(keys)
    local victim = EntIndexToHScript(keys.target_entindex);
    local attacker = EntIndexToHScript(keys.attacker_entindex);
    --print("####OnKillHero");
    -- Check if the attacker is a bot and has made a kill
    if IsPlayerBot(attacker:GetPlayerID()) and GetHeroKills(attacker:GetPlayerID()) > 0 then
        -- Get the player IDs of the teams
        local radiantPlayers = GetTeamPlayers(DOTA_TEAM_GOODGUYS)
        local direPlayers = GetTeamPlayers(DOTA_TEAM_BADGUYS)

        -- Determine which team the victim was on
        local victimTeam = DOTA_TEAM_NEUTRALS
        for _, playerID in pairs(radiantPlayers) do
            if victim:GetPlayerID() == playerID then
                victimTeam = DOTA_TEAM_GOODGUYS
                break
            end
        end
        for _, playerID in pairs(direPlayers) do
            if victim:GetPlayerID() == playerID then
                victimTeam = DOTA_TEAM_BADGUYS
                break
            end
        end


        -- Check if the victim was a bot and say a message in all chat
        --if not IsPlayerBot(victim:GetPlayerID()) then
            local message = "I just killed someone on the enemy team!"
            if attacker:GetTeam() == victimTeam then
                message = "I just killed someone on my own team!"
            end
...
-- this takes care of what to say , so far only 3 options Offensive/Defensive/Neutral lines depending if bot team have more kills than enemy
function DoTrashTalk()
	if GetBot():IsInvulnerable() or not GetBot():IsHero() or not string.find(GetBot():GetUnitName(), "hero") or  GetBot():IsIllusion() then
    	return;
	end
	--print("DOING tTalks")
	local radiantKills = 0
	local direKills = 0
	local message = "";
	for i=0, 9 do
	    --if IsPlayerBot(i) or IsPlayerHuman(i) then
	        local team = GetTeamForPlayer(i)
	        local kills = GetHeroKills(i)
	        if team == TEAM_RADIANT then
	            radiantKills = radiantKills + kills
	        elseif team == TEAM_DIRE then
	            direKills = direKills + kills
	        end
	   -- end
	end

	local botTeam = GetTeam();
	--print("leng" ..#tTalks.Trashtalks.Offensive);	
	if botTeam == 2 then
		if radiantKills > direKills then
			message = getOffensiveMessage();
		elseif radiantKills == direKills then
			message = getNeutralMessage();
		else
			message = getDefensiveMessage();
		end
	elseif botTeam == 3 then
		if radiantKills < direKills then
			message = getOffensiveMessage();
		elseif radiantKills == direKills then
			message = getNeutralMessage();
...
function AbilityLevelUpThink()  
	...
	killTracker.CheckForKillsAndDeaths(bot);
	--pingUtil.CheckForRetreatPings(bot);
	--courierUtil.CourierThink(bot);

  	--local kills = PlayerResource:GetKills(playerId);
	--if kills > 0 then
	  -- Bot has made a kill, say something in all chat
	  --local message = "I just killed someone!";
	...
	-- if DotaTime() < 30 and bot:IsHero() then
	-- 	DebugAbilityInitialization(bot)
	-- end


	-- COMPREHENSIVE THREAT SYSTEM (replaces old priority target system)
	if bot:IsAlive() and not mutil.SafeIsChanneling(bot) and not bot:IsUsingAbility() and not bot:IsMuted() then
		--print("[THREAT_SYSTEM] " .. bot:GetUnitName() .. " - Analyzing comprehensive threats")
		
		local threat, threatType, threatData = mutil.GetComprehensiveThreat(bot, 1200)
		
		if threat ~= nil or threatType == "danger_zone" then
			--print("[THREAT_SYSTEM] Threat detected - Type: " .. threatType)
			
			if threatType == "finishable_hero" then
				--print("[THREAT_SYSTEM] Attacking finishable hero: " .. threat:GetUnitName())
				bot:Action_AttackUnit(threat, false)
				return
				
			elseif threatType == "channeling_threat" then
				--print("[THREAT_SYSTEM] Responding to channeling threat: " .. threat:GetUnitName())
				local canInterrupt, interruptSource = mutil.CanInterruptChanneling(bot, threat)
				
				if canInterrupt then
					--print("[THREAT_SYSTEM] Interrupting with: " .. interruptSource:GetName())
					local behavior = interruptSource:GetBehavior()
					
					if mutil.CheckFlag(behavior, ABILITY_BEHAVIOR_UNIT_TARGET) then
						bot:Action_UseAbilityOnEntity(interruptSource, threat)
					elseif mutil.CheckFlag(behavior, ABILITY_BEHAVIOR_POINT) then
						bot:Action_UseAbilityOnLocation(interruptSource, threat:GetLocation())
					else
						bot:Action_UseAbility(interruptSource)
					end
					return
				else
					--print("[THREAT_SYSTEM] Cannot interrupt, evacuating from channeling threat")
					local evacuationLoc = mutil.GetEvacuationLocation(bot, threat:GetLocation())
					bot:Action_MoveToLocation(evacuationLoc)
					return
				end

			elseif threatType == "chain_frost_self_hit" then
				--print("[THREAT_SYSTEM] Bot was hit by Chain Frost - dispersing from allies")
				local disperseLocation = mutil.GetChainFrostDispersionLocation(bot, threat, threatData)
				bot:Action_MoveToLocation(disperseLocation)
				return
				
			elseif threatType == "chain_frost_dispersion" then
				--print(("[THREAT_SYSTEM] Dispersing from Chain Frost target: " .. threat:GetUnitName())
				local disperseLocation = mutil.GetChainFrostDispersionLocation(bot, threat, threatData)
				bot:Action_MoveToLocation(disperseLocation)
				return

			elseif threatType == "hit_based_threat" then
				--print("[THREAT_SYSTEM] Attacking hit-based threat: " .. threat:GetUnitName())
				bot:Action_AttackUnit(threat, false)
				return

			elseif threatType == "phoenix_egg_attack" then
				-- Add cooldown check ONLY for Phoenix attacks
				local phoenixAttackCooldown = mutil.phoenixLastAttackTime or 0
				if DotaTime() - phoenixAttackCooldown >= 1.0 then
					--print("[THREAT_SYSTEM] Committing to attacking Phoenix egg area")
					bot:Action_AttackMove(bot:GetLocation())
					mutil.phoenixLastAttackTime = DotaTime()  -- Set cooldown
					return
				else
					--print("[THREAT_SYSTEM] Phoenix attack on cooldown")
					return
				end
				
			elseif threatType == "danger_zone" then
				--print("[THREAT_SYSTEM] Evacuating from danger zone: " .. threatData)
				local evacuationLoc = mutil.GetEvacuationLocation(bot, bot:GetLocation())
				bot:Action_MoveToLocation(evacuationLoc)
				return
				
			elseif threatType == "hp_based_threat" then
				--print("[THREAT_SYSTEM] Attacking HP-based threat: " .. threat:GetUnitName())
				bot:Action_AttackUnit(threat, false)
				return
			end
		else
			--print("[THREAT_SYSTEM] No threats detected")
		end
end
	...
	--this takes care of when to use trashtalk
	-- if bot:IsHero() then
	-- 	if DotaTime() == (randMinA*60)+randSecA and trashTalkedOnStart == false then
	-- 		--print("TIME TO TRASH " ..tostring(bot:GetUnitName()));
	-- 		trashTalkedOnStart = true;
	-- 		DoTrashTalk();
	-- 	end

	-- 	if DotaTime() == (randMinB*60)+randSecB and trashTalkedOnBegining == false then
	-- 		trashTalkedOnBegining = true;
	-- 		DoTrashTalk();
	-- 	end

	-- 	if DotaTime() == (randMinC*60)+randSecC and trashTalkedInMid == false then
	-- 		trashTalkedInMid = true;
	-- 		DoTrashTalk();
	-- 	end

	-- 	if DotaTime() == (randMinD*60)+randSecD and trashTalkedOnEnd == false then
	-- 		trashTalkedOnEnd = true;
	-- 		DoTrashTalk();
	-- 	end
	-- end	
	---------
	...
	--     return;
	-- else
	--     if bot:IsAlive() and bot:IsHero() and GetEnemyAtTheGates() ~= false then
	--         if DotaTime() >= lastAttackCheck + 5 then
	--             local enemyLoc = GetEnemyAtTheGates();
	--             print("[DEFENSE] " .. bot:GetUnitName() .. " detected enemy at ancient: " .. tostring(enemyLoc));
	--             print("[DEFENSE] Current mode: " .. tostring(bot:GetActiveMode()) .. " desire: " .. tostring(bot:GetActiveModeDesire()));
	            
	--             local closestT2Tower = GetTower(GetTeam(),GetClosestT2TowerLocation(enemyLoc));
	--             if closestT2Tower == nil or not closestT2Tower:IsAlive() then
	--                 print("[DEFENSE] T2 tower down, sending " .. bot:GetUnitName() .. " to defend");
	--                 bot:Action_AttackMove(enemyLoc);
	--             else
	--                 print("[DEFENSE] T2 tower still alive, not defending yet");
	--             end
	...
	if bot:GetAbilityPoints() > 0 and bot:GetLevel() <= 30 then
		local lastIdx = #bot.abilities;
		
		-- Safety check: make sure we have abilities and valid index
		if bot.abilities and lastIdx > 0 and bot.abilities[lastIdx] then
			local abilityName = bot.abilities[lastIdx];
			
			-- Safety check: make sure ability name is not empty
			if abilityName and abilityName ~= "" and abilityName ~= "-1" then
				local ability = SafeGetAbilityByName(bot, abilityName);
				
				if ability ~= nil and ability:CanAbilityBeUpgraded() and ability:GetLevel() < ability:GetMaxLevel() then
					if bot:GetUnitName() == "npc_dota_hero_troll_warlord" and abilityName == "troll_warlord_whirling_axes_ranged" and ability:IsHidden() then
						bot:ActionImmediate_LevelAbility("troll_warlord_whirling_axes_melee");
					elseif bot:GetUnitName() == "npc_dota_hero_keeper_of_the_light" and abilityName == "keeper_of_the_light_illuminate" and bot:HasScepter() then
					...
					elseif ability:IsHidden() then
						...
						--print("leveling up : " ..tostring(bot:GetUnitName()) .."index: " .. tostring(lastIdx).. "abilities : " .. tostring(bot.abilities))
					end	
					bot.abilities[lastIdx] = nil;
				end
			else
				-- Remove invalid ability name and continue
				print("[ABILITY_LEVELING] Removing invalid ability name for " .. bot:GetUnitName() .. " at index " .. lastIdx);
				bot.abilities[lastIdx] = nil;
			end
		end
	end
...
function CourierUsageThink()
	if bot:IsInvulnerable() or not bot:IsHero() or bot:IsIllusion() or bot:HasModifier("modifier_arc_warden_tempest_double") or GetNumCouriers() == 0 then
		return;
	end	
	...
	-- end
		...
		-- Simple Turbo mode courier logic
	if GetGameMode() == 23 then
		TurboModeCourierThink();
		return;
	end
	...
	-- end
		...
		local courierPHP =  mutil.SafeGetHealthPercent(npcCourier); 
...
local lastCourierAction = -90;

-- Initialize courier assignment variables (these should be global for the bot)
if bot.courierAssigned == nil then bot.courierAssigned = false; end
...
function IsTargetedByUnit(courier)
	...
	local tower = GetTower(GetOpposingTeam(), i)
		if tower ~= nil and mutil.SafeGetAttackTarget(tower) == courier then
			return true;
		end
	end
	for i,id in pairs(GetTeamPlayers(GetOpposingTeam())) do
...
function ShouldTP()
	...
	then
		if mutil.SafeGetHealthPercent(bot) < 0.15 and mutil.SafeWasRecentlyDamaged(bot, 2.0) and enemies == 0 then
			tpLoc = mutil.GetTeamFountain();
		elseif mutil.SafeGetHealthPercent(bot) < 0.25 and mutil.SafeWasRecentlyDamaged(bot, 3.0) and CanJuke() == true then
...
function UnImplementedItemUsage()
	...
	if mutil.SafeIsChanneling(bot) or bot:IsUsingAbility() or bot:IsInvisible() or bot:IsMuted( ) or bot:HasModifier("modifier_doom_bringer_doom") then
		return;
	end
	...
	if pt~=nil and pt:IsFullyCastable() then
		if mode == BOT_MODE_RETREAT and pt:GetPowerTreadsStat() ~= ATTRIBUTE_STRENGTH and mutil.SafeWasRecentlyDamaged(bot, 5.0) then
	...
	if hl~=nil and hl:IsFullyCastable() then
		if ( mode == BOT_MODE_RETREAT and 
			...
			( mutil.SafeGetHealthPercent(bot)) < 0.15 ) and
	...
	if ff~=nil and ff:IsFullyCastable() then
		if ( mode == BOT_MODE_RETREAT and 
			bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and 
			bot:DistanceFromFountain() > 0 and
			( mutil.SafeGetHealthPercent(bot) ) < 0.15 ) or DotaTime() > 10*60
		then
	...
	if bst ~= nil and bst:IsFullyCastable() then
		if  mode == BOT_MODE_RETREAT and 
			bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and 
			(  mutil.SafeGetHealthPercent(bot)) < 0.10 - ( bot:GetLevel() / 500 ) and
	...
	if sc~=nil and sc:IsFullyCastable() then
		...
		for _,Ally in pairs(Allies) do
			if Ally:GetUnitName() ~= bot:GetUnitName() and not Ally:HasModifier('modifier_item_solar_crest_armor_reduction') and
			   ( (  mutil.SafeGetHealthPercent(Ally) < 0.35 and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 and CanCastOnTarget(Ally) ) or 
    ...
    if hood~=nil and hood:IsFullyCastable() and  mutil.SafeGetHealthPercent(bot)<0.8 and not bot:HasModifier('modifier_item_pipe_barrier')
	then
	...
	then
		if  not bot:HasModifier('modifier_item_lotus_orb_active') 
			...
			and ( bot:IsSilenced() or ( tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 and  mutil.SafeGetHealthPercent(bot) < 0.35 + (0.05*#tableNearbyEnemyHeroes) ) )
	    then
	...
	then
		...
		for _,Ally in pairs(Allies) do
			if  not Ally:HasModifier('modifier_item_lotus_orb_active') 
				...
				and mutli.SafeWasRecentlyDamaged(Ally,2.0)
			    and ((  mutil.SafeGetHealthPercent(Ally)< 0.35 and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 )  or 
				IsDisabled(Ally))
			then
	...
	if glimer~=nil and glimer:IsFullyCastable() then
		...
		for _,Ally in pairs(Allies) do
			if not Ally:HasModifier('modifier_item_glimmer_cape') 
			   and not Ally:IsMagicImmune()
			   and mutil.SafeWasRecentlyDamaged(Ally,2.0)
			   and ((  mutil.SafeGetHealthPercent(Ally) < 0.35 and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 0 ) or IsDisabled(Ally))
			then
	...
	then
		...
		if #tableNearbyCreeps >= 2 then
			...
			do
				local CreepHP = mutil.SafeGetHealth(creeps);
				if CreepHP > maxHP and (  mutil.SafeGetHealthPercent(creeps) ) > .75  and not creeps:IsAncientCreep()
				then
					NCreep = creeps;
					maxHP = CreepHP;
				end
	...
	if guardian~=nil and guardian:IsFullyCastable() then
		local Allies=bot:GetNearbyHeroes(1000,false,BOT_MODE_NONE);
		for _,Ally in pairs(Allies) do
			if   mutil.SafeGetHealthPercent(Ally) < 0.35 and tableNearbyEnemyHeroes~=nil and #tableNearbyEnemyHeroes > 0 
			then
	...
	if satanic~=nil and satanic:IsFullyCastable() then
		if   mutil.SafeGetHealthPercent(bot) < 0.50 and 
	...
	then
		...
		then
			if mutil.IsValidTarget(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, bot, 900) 
			   and npcTarget:HasModifier("modifier_item_spirit_vessel_damage") == false and  mutil.SafeGetHealthPercent(npcTarget) < 0.65
			then
			...
			for _,Ally in pairs(Allies) do
				...
				mutil.SafeGetHealthPercent(Ally) < 0.35 and #tableNearbyEnemyHeroes == 0 and mutil.SafeWasRecentlyDamaged(Ally, 2.5) == false   
				then
	...
	local null=IsItemAvailable("item_blood_grenade");
	if null~=nil and null:IsFullyCastable() 
	then
		if mutil.IsGoingOnSomeone(bot)
		then	
			if mutil.IsValidTarget(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, bot, 800) 
			then
			    bot:Action_UseAbilityOnLocation(null, npcTarget:GetLocation());
				return;
			end
		end
	end
	...
	-- 	if ( mode == BOT_MODE_RETREAT and 
	-- 		bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
	-- 		bot:mutil.SafeWasRecentlyDamaged(3.0) )
	-- 	then
	...
	--[[
		...
		if elix ~= nil and elix:IsFullyCastable() then
			...
			for i=1, #allies do
				if allies[i]:mutil.SafeWasRecentlyDamaged(4.0) == false and allies[i]:HasModifier('modifier_elixer_healing') == false 
	...
	if shov ~= nil and shov:IsFullyCastable() and mutil.SafeWasRecentlyDamaged(bot, 5.0) == false then
	...
	if ess_ring ~= nil and ess_ring:IsFullyCastable() then
		...
		if ( mode == BOT_MODE_RETREAT and 
			bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
			(  mutil.SafeGetHealthPercent(bot) ) < 0.25 ) and
			mutil.SafeWasRecentlyDamaged(bot, 3.0)
		then
	...
	-- 		bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and 
	-- 		bot:DistanceFromFountain() > 0 and
	-- 		bot:mutil.SafeWasRecentlyDamaged(3.0) and
	...
	if flick~= nil and flick:IsFullyCastable() then
		if ( mode == BOT_MODE_RETREAT and 
			bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
			mutil.SafeWasRecentlyDamaged(bot,3.0) and bot:IsRooted() == false )
		then
	...
	-- 	if ( mode == BOT_MODE_RETREAT and 
	-- 		bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
	-- 		bot:SafeWasRecentlyDamaged(1.0) )
	-- 	then
	-- 		bot:Action_UseAbility(ill_cape);
	-- 		return;
	-- 	end
	...
	if min_horn ~= nil and min_horn:IsFullyCastable() and bot:IsMagicImmune() == false then
		local nCastRange = bot:GetAttackRange();
		if mutil.IsGoingOnSomeone(bot) and mutil.SafeWasRecentlyDamaged(bot,2.0)
		then	
		...
		if ( mode == BOT_MODE_RETREAT and 
			bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
			mutil.SafeWasRecentlyDamaged(bot, 1.0) )
		then
			bot:Action_UseAbility(min_horn);
			return;
		end
	...
	if fboot ~= nil and fboot:IsFullyCastable() then
		if ( mode == BOT_MODE_RETREAT and 
			bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
			mutil.SafeWasRecentlyDamaged(bot,1.0) and
	...
	-- 	if ( mode == BOT_MODE_RETREAT and 
	-- 		bot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH and
	-- 		bot:SafeWasRecentlyDamaged(0.5) )
	-- 	then
	...
	if fsky~=nil and fsky:IsFullyCastable() then
		...
		then
			...
			if ( mutil.SafeWasRecentlyDamaged(bot,2.0) or bot:WasRecentlyDamagedByTower(2.0) or ( tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes > 1  ) )
			then
				local loc = mutil.GetEscapeLoc();
				bot:Action_UseAbilityOnLocation(fsky, bot:GetXUnitsTowardsLocation( loc, nCastRange ));
				return;
...
function IsTargetedByEnemy(building)
	...
	do
		if ( GetUnitToUnitDistance(building, hero) <= hero:GetAttackRange() + 200 and mutil.SafeGetAttackTarget(hero) == building ) then
			return true;
		end
	end
	return false;
...
function UseGlyph()
	...
	do
		...
		if  tower ~= nil and mutil.SafeGetHealthForBuilding(tower) > 0 and  mutil.SafeGetHealthPercentForBuilding(tower)< 0.15 and mutil.SafeGetAttackTarget(tower) ~=  nil
		then
	...
	do
		...
		if barrack ~= nil and mutil.SafeGetHealthForBuilding(barrack) > 0 and  mutil.SafeGetHealthPercentForBuilding(barrack) < 0.5 and IsTargetedByEnemy(barrack)
		then
			bot:ActionImmediate_Glyph( )
			return
		end
	...
	local Ancient = GetAncient(GetTeam())
	if Ancient ~= nil and mutil.SafeGetHealthForBuilding(Ancient) > 0 and  mutil.SafeGetHealthPercentForBuilding(Ancient)< 0.5 and IsTargetedByEnemy(Ancient)
	then
		bot:ActionImmediate_Glyph( )
		return
	end
...
function ItemUsageThinks()
	...
	then
		if ( mode == BOT_MODE_RETREAT and modeDesire >= BOT_MODE_DESIRE_HIGH 
		     and bot:DistanceFromFountain() > 0 and mutil.SafeGetHealth(bot) < 0.15*mutil.SafeGetHealth(bot) ) 
			 or DotaTime() > 10*60 
		then
			bot:Action_UseAbility(itemToUse);
			return;
	...
	if test ~= false then
		if test:GetToggleState() == true and mutil.SafeGetHealth(bot) <= 150 and DotaTime() > lastToggle + 0.5 then
...
function ItemUsageThink()
	...
	if bot:IsAlive() == false 
	   ...
	   or mutil.SafeIsChanneling(bot) == true
